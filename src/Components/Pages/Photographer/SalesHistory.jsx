...
const PhotographerSales = () => {
  ...
  const fetchSales = async () => {
    try {
      ...
      // Get all receipts
      const receiptsRes = await axios.get(`${API}/payments/admin/receipts`, { headers });
      
      // Filter receipts that contain photographer's media
      const myMediaRes = await axios.get(`${API}/media`, { headers });
      const myMediaIds = (myMediaRes.data || [])
        .filter(m => m.userId === photographerId)
        .map(m => m.id);

      const mySales = (receiptsRes.data || []).filter(r => 
        r.items?.some(item => myMediaIds.includes(item.mediaId))
      );
